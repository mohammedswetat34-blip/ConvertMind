...
- **Scoring calibration**: the prompt carries an explicit band rubric (90-100 world-class … 0-34 broken) and forbids central-tendency compression. The model scores **five dimensions** (trust, conversion, psychology, copy, mobile) plus a **`confidence`** (0-100); it no longer emits `overall`. `issues` is now 1–4 *real* issues (no forced quota / forced "critical").
- **Server-side validation** (`validateReport`): every report is normalized before caching/serving — strings length-clamped, enums whitelisted (invalid → safe default), scores clamped 0–100, unknown fields dropped. **`overall` is computed** here as a weighted blend (`conversion .30, trust .25, copy .20, psychology .15, mobile .10`) — never taken from the model. Invalid shape → **one semantic retry** (budget-guarded), then 500. Responses are **built field-by-field** (`prepareTierView`) — model output is never cloned, so unknown properties cannot reach any tier.
- **Confidence ceiling**: after validation, `confidence` is capped server-side to ≤35 for thin content and ≤20 for failed fetches, so a low-signal audit can never present as authoritative. The frontend shows a "limited analysis" note when confidence < 50.
...
**Response 200** — tier-shaped report object (see [analysis-flow.md](analysis-flow.md)) plus `"tier": "pro"|"free"`, `"confidence": 0-100`, `scores.overall` (computed), and, on a cache hit, `"cached": true`.
