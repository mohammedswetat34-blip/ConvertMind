...
Four stateless serverless functions in `api/`. All share the same CORS pattern (`ALLOWED_ORIGIN` env var, OPTIONS preflight → 204) and the same `getClientIp` helper (prefers unspoofable `x-real-ip`).
...
4. **Cache check**: same href scanned within 1h → served from per-instance memory (no Claude cost; daily caps not charged). Cache holds the full report; tier view is applied per request.
...
**Processing**
- `safeFetch` (12s abort, manual-redirect with per-hop SSRF re-check) → strip HTML → 8,000 chars.
- Claude call: native `https`, `claude-haiku-4-5-20251001`, `max_tokens: 3000`, 25s timeout. Prompt demands verbatim-quote evidence in issues and a ready-to-use example in action item #1.
- `extractJSON` 4-strategy parse.
...
```
Values come from `STRIPE_PRO_URL` / `STRIPE_AGENCY_URL`; `null` if unset. No rate limit (public, cheap). Non-GET → 405.
...
**Behavior**
- If `RESEND_API_KEY` set → sends a branded confirmation email via Resend (failure is non-fatal, logged).
- If not set → just `console.log`s the signup.
...
3. `email` required, contains `@`, ≤254 chars. `url` must parse as http/https (else 400).
4. All content strings are length-clamped and HTML-escaped server-side; scores clamped 0–100; counts clamped 0–20.
